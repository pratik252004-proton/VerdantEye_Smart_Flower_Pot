/* 
  VerdantEye - Full sketch with custom AP Wi-Fi setup page
  - Serves custom HTML at AP mode
  - Saves credentials to Preferences
  - Connects to Wi-Fi and runs RoboEyes + WeatherStation
*/
...
// Weather API settings (keep these static or change as needed)
...
// HTML/CSS (VerdantEye themed). Serves on root "/"
...
// handle POST from form, save SSID & password to Preferences
void handleSave() {
  ...
  delay(1200); // let the client receive the response
  ESP.restart();
}

// Clears stored credentials (for reconfiguration)
void handleClear() {
  preferences.begin("wifi", false);
  preferences.clear();
  preferences.end();
...
// Start AP: ensures WiFi mode is AP and starts softAP
void startAPMode() {
  Serial.println("Starting AP mode...");
  WiFi.disconnect(true, true);             // clear station settings and disconnect
  delay(200);
  WiFi.mode(WIFI_AP);                      // explicitly set AP
  delay(50);

  const char* apName = "VerdantEye_Setup";
  const char* apPass = "12345678";         // choose your own if you want

  bool ok = WiFi.softAP(apName, apPass);
  if (!ok) {
...
// Try to read stored credentials and connect (returns true on success)
...
void initRoboEyesAndWeather() {
  ...
  // WeatherStation init (use the connected WiFi to fetch)
  preferences.begin("wifi", true);
  String ssid = preferences.getString("ssid", "");
  String pass = preferences.getString("pass", "");
  preferences.end();

  // weatherStation API expects Wi-Fi details, but many libs don't actually need them after connection.
...
void setup() {
  ...
  digitalWrite(LED_PIN, HIGH); // LED on while starting

  // Initialize display early so the user sees something if it's connected
  if (!display.begin(0x3C, true)) {
    Serial.println(F("SH1106 allocation failed"));
    // continue anyway — user might not have display attached
  } else {
  ...
  if (!connected) {
    // Start AP mode (serve page) so user can configure
    startAPMode();
    ...
    // On successful WiFi connection initialize services
    initRoboEyesAndWeather();
    currentMode = IDLE;
    modeStartTime = millis();
    digitalWrite(LED_PIN, LOW); // turn LED off to indicate ready
    Serial.println("Ready and running in normal mode.");
  }
}

void loop() {
  // If AP mode: serve captive page
  if (WiFi.getMode() == WIFI_AP) {
    server.handleClient();
    // optional: small LED blink to show AP mode
  ...
  // Not AP: normal operation (RoboEyes + Weather)
  ...
  // If WiFi drops, go to AP mode for reconfiguration
  if (WiFi.status() != WL_CONNECTED) {
